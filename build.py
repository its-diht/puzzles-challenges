...
(function () {{
  ...
  // no-ops here because this page has no #stage-data element.
  const {{ normalize, b64ToBytes, deriveKey, IV_LEN }} = window.ARGCrypto;

  async function tryCandidate(candidate, password) {{
    const salt = b64ToBytes(candidate.salt);
    const key = await deriveKey(password, salt, candidate.iterations);
  ...
  async function tryUnlock(rawKey, errorEl, statusEl, linkEl, labelEl, entrySection, resultSection) {{
    const password = normalize(rawKey);
    for (const candidate of CANDIDATES) {{
      try {{
        const plainBuf = await tryCandidate(candidate, password);
        ...
        // Wrong key for this candidate -- move on and try the next one.
      }}
    }}
    errorEl.textContent = "Incorrect key.";
    return false;
  }}
